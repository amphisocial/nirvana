...
async function initialize() {
  ...
  $('#accountForm').addEventListener('submit', async (event) => {
    ...
    button.disabled = true;
    try {
      await api('/api/accounts', { method: 'POST', body: JSON.stringify(formObject(event.currentTarget)) });
  ...
  $('#csvForm').addEventListener('submit', async (event) => {
    ...
    button.disabled = true;
    try {
      const result = await api('/api/accounts/holdings/import', { method: 'POST', body: new FormData(event.currentTarget) });
