...
## 2. Stop any old or stuck Nirvana npm install

If the current terminal is still showing `npm ci`, press `Ctrl+C` once. Do not kill every npm process on the shared server because another AthenaBot application may be deploying.

Find only the relevant process:

```bash
pgrep -af "npm (ci|install)"
```

For each returned PID, confirm it belongs to Nirvana:

```bash
sudo pwdx <PID>
```

Only when the working directory is `/opt/apps/nirvana`, stop that PID:
...
Use `sudo kill -9 <PID>` only if the process remains after the normal stop.

Clean the partial dependency directory:
...
## 3. Install the Nirvana application files
...
Confirm the server can reach the public npm registry:
...
The repository includes `.npmrc`, which sets the registry to `https://registry.npmjs.org/`. The packaged `package-lock.json` must also contain only `registry.npmjs.org` download URLs. You can verify it with:

```bash
grep -n "applied-caas-gateway\|artifactory/api/npm" package-lock.json
```
...
If `npm ci` reports that `package.json` and `package-lock.json` are out of sync, do not let it retry indefinitely. Run this once to regenerate the lockfile from the public registry, then commit the changed lockfile:

```bash
rm -rf node_modules package-lock.json
npm install --package-lock-only --registry=https://registry.npmjs.org --ignore-scripts --no-audit --no-fund
npm ci --omit=dev --registry=https://registry.npmjs.org --no-audit --no-fund
```

If `getent` or `curl` cannot reach `registry.npmjs.org`, the problem is server DNS/network access rather than Nirvana. Fix that before rerunning npm.

## 4. Create the PostgreSQL login, database, and grants
...
## 5. Configure `/opt/apps/nirvana/.env`
...
## 7. Add only the Nirvana PM2 process
...
## 8. Add Nirvana to the existing Nginx service
...
## 9. Issue and install the Nirvana TLS certificate with Certbot
...
## 10. Configure external service URLs
